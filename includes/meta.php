...
/**
 * Register post meta (Name, Source, Source URL, Rating) and meta box.
 */
...
function tcpt_sanitize_text($value){
...
add_action('init', function(){
	...
	register_post_meta(TCPT_SLUG, 'tcpt_rating', array(
		...
		'single' => true,
		'show_in_rest' => array(
			'schema' => array('type' => 'integer', 'minimum' => 0, 'maximum' => 5),
		),
		'default' => 0,
...
// Meta box for classic & block editor
add_action('add_meta_boxes', function(){
	add_meta_box(
		'tcpt_details',
		__('Testimonial Details', TCPT_TEXTDOMAIN),
		'tcpt_render_meta_box',
		TCPT_SLUG,
		'normal',
		'high'
	);
...
function tcpt_render_meta_box($post){
	...
	<div class="tcpt-fields">
		<p>
			<label for="tcpt_name"><strong><?php esc_html_e('Name', TCPT_TEXTDOMAIN); ?></strong></label><br />
			<input type="text" id="tcpt_name" name="tcpt_name" class="widefat" value="<?php echo esc_attr($name); ?>" />
		</p>
		<p>
			<label for="tcpt_source"><strong><?php esc_html_e('Source', TCPT_TEXTDOMAIN); ?></strong> <span class="description">(<?php esc_html_e('e.g., Company or Publication', TCPT_TEXTDOMAIN); ?>)</span></label><br />
			<input type="text" id="tcpt_source" name="tcpt_source" class="widefat" value="<?php echo esc_attr($source); ?>" />
		</p>
		<p>
			<label for="tcpt_source_url"><strong><?php esc_html_e('Source URL (optional)', TCPT_TEXTDOMAIN); ?></strong></label><br />
			<input type="url" id="tcpt_source_url" name="tcpt_source_url" class="widefat" value="<?php echo esc_attr($source_url); ?>" placeholder="https://example.com" />
		</p>
		<p>
			<label for="tcpt_rating"><strong><?php esc_html_e('Rating (0–5)', TCPT_TEXTDOMAIN); ?></strong></label><br />
			<input type="number" id="tcpt_rating" name="tcpt_rating" min="0" max="5" step="1" value="<?php echo esc_attr($rating); ?>" />
		</p>
	</div>
	<?php
}
